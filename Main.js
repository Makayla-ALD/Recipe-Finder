...
async function searchRecipes(query) {
    ...
    try {
        ...
        showMessage("Somthing went wrong, Please try again.", true);
    }
}
...
function displayRecipes(recipes) {
    if (!recipes || recipes.length === 0 ) {
        showMessage("No recipes to dislpay");
        return;
    }
    ...
    recipes.forEach(recipe => {
        ...
        recipeDiv.innerHTML = ` <img src="${recipe.strMealThumb}" alt="${recipe.strMeal}" loading="lazy"> 
...
function displayRecipeDetails(recipe) {
    ...
    for (let i = 1; i <= 20; i++) {
        ...
        const measure = recipe['strMeasures${i}']?.trim();
    ...
    const sourceHTML = recipe.strSource ? `<div class="source-wrapper"><a href="${recipe.strSource}" target="_blank">View Original Source</ad></div>` : "";

    modalContent.innerHTML = `<h2>${recipe.strMeal}</h2>
    <img src="${recipe.strMealThumb}" alt="${recipe.strMeal}"> 
    ${categoryHTML} ${areaHTML} ${ingredientsHTML} ${instructionsHTML} ${youtubeHTML} ${sourceHTML}`;
}